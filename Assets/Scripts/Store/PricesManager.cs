...
public class PricesManager : MonoBehaviour
{
    ...
    private List<IStorage> _registeredStorages = new List<IStorage>();
    ...
    public void RegisterStorage(IStorage storage) => _registeredStorages.Add(storage);
    public void UnregisterStorage(IStorage storage) => _registeredStorages.Remove(storage);
    ...
    {
        foreach (var storage in _registeredStorages)
        {
            storage.RefreshPriceFromManager();
        }
    }
